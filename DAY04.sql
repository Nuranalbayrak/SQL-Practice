...
select * from meslekler offset 2 row limit 5
...
SELECT actor_name,film_name FROM aktorler AS Y
LEFT JOIN filmler AS X
ON X.film_id=Y.film_id;

SELECT actor_name,film_name FROM filmler AS Y
right JOIN aktorler AS X
ON X.film_id=Y.film_id;
